...
class SearchEngine(TextPrecessor):
    ...
    def search_one_words_cluster(self):
        search_word_list=super().tokenize_and_stem(self.search_input)
        # loop over each word serach
        for word_ in search_word_list:
            # Using loops
            counter = {key: 0 for key in self.top_terms_per_cluster.keys()}

            # Loop over each self.top_terms_per_cluster and count occurrences of search_word
            for key, value in self.top_terms_per_cluster.items():
                for word in value:
    ...
    def complex_search(self):
        ...
        synonyms = {}
        for term in re.findall(r'\b\w+\b', self.search_input):
            synonyms[term] = set()
            for syn in wordnet.synsets(term):
                for lemma in syn.lemmas():
                    synonyms[term].add(lemma.name())

        or_keywords = [k.strip() for k in self.search_input.split("and")]
        and_keywords = []
        for match in re.findall(r'\((.*?)\)|(\b\w+\b)|(\b(or)\b)|(\b(and)\b)', self.search_input):
